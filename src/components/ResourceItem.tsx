...
function ResourceItem() {
    const ResourcesList = [
        ...
        },
        {
            ...
            title: "BibleProject",
            desc: "Free online resources, including videos and other media tools, to help learn and study the Bible.",
    ...
    return (
        ...
        {ResourcesList.map((item, index) => (
            <Grid.Col key={index} span={{ base: 7, md: 3, lg: 3 }}>
            <Flex target="_blank" className={classes.a} component="a" href={item.link} px={2} align="center" justify="center" direction="column"> 
                ...
                <Text hiddenFrom="md" mt="sm" size="sm"  fw={600}>{item.title}</Text>
                <Text visibleFrom="md" mt={4} size="sm">{item.desc}</Text>
                <Text hiddenFrom="md" mt={4} size="xs">{item.desc}</Text>
            </Flex>
        </Grid.Col>
        ))}
        </>
